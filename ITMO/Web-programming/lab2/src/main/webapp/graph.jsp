...
%>
<script>
    const results = <%= new com.google.gson.Gson().toJson(results) %>;
</script>
...
<script charset="utf-8" type="text/javascript">
    ...
    graph.addEventListener("click", event => {
        ...
        selectedRs.forEach(r => {
            sendRequest(gX, gY, String(r), flagFromGraph);
        });
    });
    ...
    document.querySelectorAll('input[name="R"]').forEach(checkbox => {
        checkbox.addEventListener("change", updateGraph(results));
    });
    ...
    function getSelectedRs() {
        ...
        return Array.from(rCheckboxes).map(checkbox => checkbox.value);
    }
</script>
</body>
</html>
